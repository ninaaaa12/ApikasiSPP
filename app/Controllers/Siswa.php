...
class Siswa extends BaseController
{
	...
	public function log()
	{
		...
		if ($this->validate($validasiform)) {
			...
			$cekLogin = $this->siswa->where($whereLogin)->findAll();

			if (count($cekLogin) == 1) {
				$dataSession = [
					'nisn' => $cekLogin[0]['nisn'],
		...
		return view('/v_logsiswa');
	}

	public function dashboard()
	{
		return view('/dashboadsiswa');
	}
	...
	public function history()
	{
		$this->bayar->join('siswa', 'siswa.nisn=pembayaran.nisn');
		$this->bayar->join('kelas', 'kelas.id_kelas=siswa.id_kelas');
		$data['listPembayaran'] = $this->bayar->where('pembayaran.nisn', session()->get('nisn'))->findAll();


		return view('/vsiswa/history-pembayaran', $data);
	}
	public function home()
	{
		return view('/vsiswa/siswa');
	}
