...
class Pembayaran extends BaseController
{
	...
	public function simpanBayar()
	{
		...
		if ($jumlahBayar > $this->request->getPost('txtTarifSpp')) {
			...
			$pembayaran =  $this->bayar->where('nisn ', $dataPembayaran['nisn'])->findAll();
			if (count($pembayaran) == 1) {
				return redirect()->to('/bayar')->with('pesan-error', '<civ class="alert alert-danger">Siswa sudah bayar di bulan ini</div>');
			} else {
